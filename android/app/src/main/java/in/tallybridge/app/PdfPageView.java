...
final class PdfPageView extends View {
 private final int backdrop;private Bitmap bitmap;private final Paint paint=new Paint(Paint.ANTI_ALIAS_FLAG|Paint.FILTER_BITMAP_FLAG);
 private float zoom=1,dx=0,dy=0,lastX,lastY;private final ScaleGestureDetector scale;
 PdfPageView(Context context,Bitmap b,int background){super(context);backdrop=background;bitmap=b;setContentDescription("PDF page. Pinch to zoom, drag to pan.");scale=new ScaleGestureDetector(context,new ScaleGestureDetector.SimpleOnScaleGestureListener(){@Override public boolean onScale(ScaleGestureDetector d){zoom=Math.max(1,Math.min(4,zoom*d.getScaleFactor()));clamp();invalidate();return true;}});}
 ...
 @Override protected void onDraw(Canvas c){super.onDraw(c);if(bitmap==null)return;c.drawColor(backdrop);clamp();float s=fit()*zoom;c.save();c.translate((getWidth()-bitmap.getWidth()*s)/2+dx,(getHeight()-bitmap.getHeight()*s)/2+dy);c.scale(s,s);c.drawBitmap(bitmap,0,0,paint);c.restore();}
